...
const FolderImages = () => {
    ...
    const [prevDisplay, setPrevDisplay] = useState(5);
    const previewStart = Math.max(0, Math.min(currentImage - Math.floor(prevDisplay / 2), images.length - prevDisplay));
    ...
    const handlePrev = () => {
        console.log(currentImage, previewStart);
        setCurrentImage(prev => (prev === 0 ? images.length - 1 : prev - 1));
    };

    const handleNext = () => {
        console.log(currentImage, previewStart, images.length);
    ...
    useEffect(() => {
        ...
        updatePrevDisplay();
        window.addEventListener("resize", updatePrevDisplay);
        return () => window.removeEventListener("resize", updatePrevDisplay);
    })
    ...
    return (
        <section>
            ...
            <div className="flex flex-col md:flex-row gap-4 [@media(min-width:2560px)]:gap-6">
                            ...
                            {images.slice(previewStart, previewStart + prevDisplay).map((src, idx) => (
                                <img
                                    ...
                                    className={`w-20 h-20 object-cover rounded cursor-pointer ${
                                        currentImage === previewStart + idx ? 'ring-2 ring-red-500' : ''
                                    }`}
                                />
                            ))}
                        </div>
                        <ArrowForwardIosIcon className="z-10 text-black" onClick={handleNext}></ArrowForwardIosIcon>
                    </div>
